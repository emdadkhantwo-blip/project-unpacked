...
import { ClipboardList, Clock, CheckCircle2, AlertTriangle, TrendingUp } from "lucide-react";
import { Skeleton } from "@/components/ui/skeleton";

interface HousekeepingStaffDashboardProps {
...
export function HousekeepingStaffDashboard({
  assignedCount,
  pendingCount,
  inProgressCount,
  completedTodayCount,
  highPriorityCount,
  isLoading,
}: HousekeepingStaffDashboardProps) {
  if (isLoading) {
    return (
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Your housekeeping task overview</p>
            </div>
          </div>
          {assignedCount > 0 && (
            <Badge variant="secondary" className="bg-emerald-500/20 text-emerald-700">
              {assignedCount} Active {assignedCount === 1 ? 'Task' : 'Tasks'}
            </Badge>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <div className="bg-background/60 rounded-lg p-4 text-center">
            <div className="flex items-center justify-center gap-2 mb-2">
              ...
              <span className="text-2xl font-bold text-green-600">{completedTodayCount}</span>
            </div>
        ...
        {assignedCount === 0 && (
          <div className="mt-4 text-center py-4 bg-background/40 rounded-lg">
            <p className="text-sm text-muted-foreground">
              🎉 You have no tasks assigned. Great job staying on top of things!
            </p>
          </div>
        )}
      </CardContent>
